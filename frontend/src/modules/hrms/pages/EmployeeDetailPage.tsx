            ...
            {expenses && expenses.length > 0 ? (
              <div className="space-y-2">
                {expenses.map((exp: any) => {
                  const s = (exp.status || '').toUpperCase();
                  ...
                  if (s.includes('REIMBURSED') || s.includes('PAID')) {
                    ...
                    displayStatus = 'Reimbursed';
                  } else if (s.includes('FINANCE') || s === 'APPROVED') {
                    badgeClass = 'bg-emerald-100 text-emerald-800 border border-emerald-200';
                    displayStatus = 'Finance Approved';
                  } else if (s.includes('MANAGER')) {
                  ...
                  return (
                    <div key={exp.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500">{exp.description || 'No description'}</p>
                      </div>
                      <span className={`px-2.5 py-1 rounded-lg font-bold text-[10px] ${badgeClass}`}>{displayStatus}</span>
                    </div>
                  );
                })}
              </div>

...
export const LedgerPage: React.FC = () => {
  const { journalEntries, activeSubSection, setActiveSubSection } = useApp();

  // Navigation Tabs for Accounting Flow: Voucher -> Journal -> Lines -> Account Ledger -> Trial Balance -> P&L / BS
  const validLedgerTabs = ['journals', 'lines', 'account_ledger', 'trial_balance', 'reports'];
  const mainTab = (validLedgerTabs.includes(activeSubSection) ? activeSubSection : 'journals') as 'journals' | 'lines' | 'account_ledger' | 'trial_balance' | 'reports';
  const setMainTab = (tab: 'journals' | 'lines' | 'account_ledger' | 'trial_balance' | 'reports') => setActiveSubSection(tab);
